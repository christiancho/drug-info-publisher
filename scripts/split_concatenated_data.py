...
def split_json_files():
    ...
    print("Concatenating all chunk files...")
    all_content = ''
    ...
    print(f"Total concatenated length: {len(all_content)} characters")
    
    # Now try to parse the concatenated JSON
    try:
        # The content is still escaped, so decode it
        unescaped_content = all_content.encode().decode('unicode_escape')
        drugs = json.loads(unescaped_content)
        
        if isinstance(drugs, list):
            print(f"✓ Successfully parsed {len(drugs)} drugs!")
        else:
        ...
        print("First 200 chars of unescaped content:")
        try:
            unescaped_sample = all_content[:200].encode().decode('unicode_escape')
            print(repr(unescaped_sample))
        except:
    ...
    for i in range(80):
        ...
        chunk_drugs = drugs[start_idx:end_idx]
        ...
        # Show sample drug info for first few files
        if i < 3 and len(chunk_drugs) > 0:
            sample_drug = chunk_drugs[0]
            print(f"    Sample: {sample_drug.get('drugName', 'Unknown')} ({sample_drug.get('genericName', 'N/A')})")
